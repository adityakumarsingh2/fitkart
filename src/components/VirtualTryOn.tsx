...
const VirtualTryOn = () => {
    ...
    return (<section className="py-20 md:py-32 bg-gradient-subtle">
      <div className="container mx-auto px-4">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid lg:grid-cols-2 gap-8 lg:gap-12">
            {/* Left: Upload Zone */}
            <div className={cn("relative rounded-2xl border-2 border-dashed transition-all duration-300 overflow-hidden", isDragging ? "border-accent bg-accent/5" : "border-border hover:border-accent/50", uploadedImage ? "bg-card" : "bg-background")} onDragEnter={handleDragIn} onDragLeave={handleDragOut} onDragOver={handleDrag} onDrop={handleDrop}>
              {uploadedImage ? (<div className="relative aspect-[3/4]">
                  <img src={uploadedImage} alt="Uploaded" className="w-full h-full object-cover"/>
                  
                  {/* Analysis Overlay */}
                  {isAnalyzing && (<div className="absolute inset-0 bg-foreground/60 backdrop-blur-sm flex items-center justify-center">
                  ...
                  {/* Close Button */}
                  <button onClick={resetUpload} className="absolute top-4 right-4 w-10 h-10 rounded-full bg-background/90 backdrop-blur-sm flex items-center justify-center hover:bg-background transition-colors shadow-md">
                    <X className="h-5 w-5"/>
                  </button>
                </div>) : (<div className="aspect-[3/4] flex flex-col items-center justify-center p-8 text-center">
            ...
            <div className="flex flex-col justify-center">
              {analysisComplete ? (<div className="space-y-6 animate-fade-up">
            ...
            ].map((size) => (<div key={size.label} className="bg-card rounded-xl p-4 border border-border">
