...
import { Upload, Camera, Sparkles, Loader2, Check, X, ChevronLeft, ChevronRight, Lightbulb } from "lucide-react";
import WeightHeightDialog from "@/components/WeightHeightDialog";
import { getSizeRecommendation } from "@/lib/sizeRecommendation";
...
const TryOnPage = () => {
  ...
  // Check if user has weight and height, show dialog if not
  useEffect(() => {
    if (user && !authLoading) {
      const hasWeight = user.user_metadata?.weight;
      const hasHeight = user.user_metadata?.height;
  ...
  const handleSaveMeasurements = async (weight, height) => {
  ...
  const analyzeBody = async () => {
    ...
    // Get user measurements for size recommendation
    const weight = parseFloat(user.user_metadata?.weight || "0");
    const height = parseFloat(user.user_metadata?.height || "0");
    ...
    setIsAnalyzing(true);
    try {
      // Get size recommendation from our algorithm
      const sizeRec = getSizeRecommendation({ weight, height });

      // Set analysis with size recommendation
      setAnalysis({
        estimatedSize: sizeRec.size,
        bmi: sizeRec.bmi,
        bodyType: sizeRec.bodyType,
        fitType: sizeRec.fitType,
        lengthType: sizeRec.lengthType,
        fittingTips: sizeRec.tips,
        confidence: sizeRec.confidence,
        fitNotes: `Based on your measurements (${weight}kg, ${height}cm), we recommend size ${sizeRec.size}.`,
        recommendations: sizeRec.tips.slice(0, 3)
      });
              ...
              {analysis && (
                <div className="bg-card rounded-2xl border border-border p-6 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="p-4 rounded-xl bg-muted/50">
                      <p className="text-sm text-muted-foreground">Fit Type</p>
                      <p className="font-medium capitalize">{analysis.fitType}</p>
                    </div>
                    <div className="p-4 rounded-xl bg-muted/50">
                      <p className="text-sm text-muted-foreground">Length</p>
                      <p className="font-medium capitalize">{analysis.lengthType}</p>
                    </div>
                  </div>
                  ...
                  {/* Fitting Tips */}
                  {analysis.fittingTips && analysis.fittingTips.length > 0 && (
                    <div>
                      ...
                      <ul className="space-y-2">
                        {analysis.fittingTips.map((tip, i) => (
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {currentProduct.sizes?.map((size) => (
                          <span
                            key={size}
                            className={`px-3 py-1.5 rounded-lg text-sm border ${analysis?.estimatedSize === size
